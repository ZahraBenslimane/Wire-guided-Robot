...
// P0.2 -- P4.5 : input speed robot
// P0.6 -- P4.10 : input status robot
...
#define GET_SPEED_ROBOT  4 // (FIO0PIN &0x3C0) >> 6    
#define GET_STATUS_ROBOT 1  //(FIO0PIN &0x3C) >> 2 
...
void init_PWM(void)
{
    ...
    PWM1MR1 = 80* PWM1MR0 / 100;
    PWM1MR2 = 30* PWM1MR0 / 100;
...
int main (){
	...
	int num_robot= 12; int vitesse_robot = 4; int status_robot=1;
	...
	while (1){ 
        if (new_trame==1){etat =1; 
				                 if (GET_NUM_ROBOT != num_robot || GET_SPEED_ROBOT != vitesse_robot  || GET_STATUS_ROBOT != status_robot) 
